...
int main(void)
{
	...
	float sysCallbackRate = hw.AudioCallbackRate();
    ...
    while (1)
    {
        ...
        if (currentTicks - lastTicksRefresh > (100000 * ticksPerUs))
        {
            ...
            hw.PrintLine("Tick...");
            hw.PrintLine("P1: %3.5f, P2: %3.5f", potentiometerArray.analogControl[0].GetRawFloat(), potentiometerArray.analogControl[1].GetRawFloat());
            UpdateDisplay();
        }
    }
	// init hardware
